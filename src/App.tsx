...
export default function Home() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>, kind: 'customer' | 'credit' | 'payment') {
    ...
    if (saved) { notify(kind === 'customer' ? '고객을 저장했습니다.' : kind === 'credit' ? '외상을 저장했습니다.' : '입금을 저장했습니다.'); setDialog(null); return; }
    if (kind === 'customer') {
      setCustomers((list) => [...list, { id: `C${String(list.length + 1).padStart(3, '0')}`, name: String(payload.name), phone: String(payload.phone || ''), memo: String(payload.memo || ''), balance: 0, oldestDate: null, cylinderCount: 0 }]); notify('고객을 등록했습니다.');
    } else {
