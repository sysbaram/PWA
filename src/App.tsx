...
const sampleCustomers: Customer[] = [
  ...
  { id: 'C002', name: '이정희', phone: '010-9981-2046', balance: 320000, oldestDate: '2026-08-18' },
  { id: 'C003', name: '박성호', phone: '010-7102-8821', balance: 175000, oldestDate: '2026-09-02' },
  { id: 'C004', name: '최은영', phone: '010-3366-1290', balance: 80000, oldestDate: '2026-09-10' },
  { id: 'C005', name: '한지우', phone: '010-4877-3301', balance: 0, oldestDate: null },
];
...
export default function Home() {
  ...
  const [dialog, setDialog] = useState<'customer' | 'credit' | 'payment' | 'settings' | 'detail' | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`${savedUrl}?action=summary&key=${encodeURIComponent(savedKey)}&t=${Date.now()}`).then((response) => response.json()).then((data) => {
      ...
      setCustomers(data.customers); setTransactions(data.transactions); setConnected(true);
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>, kind: 'customer' | 'credit' | 'payment') {
    ...
    if (kind === 'customer') {
      setCustomers((list) => [...list, { id: `C${String(list.length + 1).padStart(3, '0')}`, name: String(payload.name), phone: String(payload.phone || ''), balance: 0, oldestDate: null }]); notify('고객을 등록했습니다.');
    } else {
  ...
  return <main className="min-h-screen bg-background pb-28 text-foreground">
    ...
    <div className="mx-auto max-w-6xl px-4 pt-5 sm:px-6 sm:pt-8">
      <section className="balance-card overflow-hidden rounded-[26px] p-5 text-white shadow-[0_18px_50px_rgba(26,64,55,.2)] sm:p-7"><div className="flex items-start justify-between"><div><p className="text-sm text-white/70">전체 받을 금액</p><p className="mt-2 text-[32px] font-bold tracking-[-.04em] sm:text-4xl">{money(total)}</p></div><span className="rounded-full bg-white/12 px-3 py-1.5 text-xs font-medium text-white/80">{receivables.length}명 미수</span></div><div className="mt-7 flex items-center gap-2 border-t border-white/15 pt-4 text-sm text-white/72"><CalendarClock className="size-4" /> 30일 이상 미수금 {money(receivables.filter((c) => ageInDays(c.oldestDate) >= 30).reduce((s, c) => s + c.balance, 0))}</div></section>
      <section className="mt-4 grid grid-cols-3 gap-2.5"><QuickAction icon={<ArrowUpRight />} label="외상 추가" tone="credit" onClick={() => setDialog('credit')} /><QuickAction icon={<ArrowDownLeft />} label="입금 처리" tone="payment" onClick={() => setDialog('payment')} /><QuickAction icon={<Users />} label="고객 등록" tone="customer" onClick={() => setDialog('customer')} /></section>
      ...
      {activeTab !== 'history' ? <section className="mt-7"><div className="mb-3 flex items-end justify-between"><div><p className="eyebrow">RECEIVABLES</p><h2 className="mt-1 text-xl font-bold tracking-tight">{activeTab === 'home' ? '미수금 고객' : '전체 고객'}</h2></div><span className="text-xs text-muted-foreground">잔액 높은 순</span></div><div className="grid gap-2.5 lg:grid-cols-2">{(activeTab === 'home' ? filtered.filter((c) => c.balance > 0) : filtered).sort((a, b) => b.balance - a.balance).map((customer) => <button key={customer.id} onClick={() => openDetail(customer)} className="group flex w-full items-center gap-3.5 rounded-[20px] border border-border/80 bg-card p-4 text-left shadow-[0_3px_14px_rgba(39,49,45,.04)] transition hover:-translate-y-0.5 hover:border-primary/25 hover:shadow-md"><span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-secondary font-bold text-secondary-foreground">{customer.name.slice(0, 1)}</span><span className="min-w-0 flex-1"><span className="flex items-center gap-2"><strong className="text-[15px]">{customer.name}</strong>{ageInDays(customer.oldestDate) >= 30 && <span className="rounded-full bg-warning/12 px-2 py-0.5 text-[10px] font-bold text-warning">{ageInDays(customer.oldestDate)}일 경과</span>}</span><span className="mt-1 block text-xs text-muted-foreground">{customer.oldestDate ? `${shortDate(customer.oldestDate)}부터 · ${customer.phone}` : `미수금 없음 · ${customer.phone}`}</span></span><span className="text-right"><strong className={customer.balance > 0 ? 'block text-[15px] text-ink-red' : 'block text-[15px] text-primary'}>{money(customer.balance)}</strong><span className="mt-1 flex items-center justify-end text-[11px] text-muted-foreground">내역 <ChevronRight className="size-3.5" /></span></span></button>)}{filtered.length === 0 && <EmptyState />}</div></section> : <section className="mt-7"><div className="mb-3"><p className="eyebrow">HISTORY</p><h2 className="mt-1 text-xl font-bold tracking-tight">최근 거래내역</h2></div><TransactionList items={visibleTransactions} /></section>}
    </div>
    ...
    {dialog && <Modal title={{ customer: '새 고객 등록', credit: '외상 추가', payment: '입금 처리', settings: 'Google Sheets 연결', detail: selected?.name || '거래내역' }[dialog]} onClose={() => setDialog(null)}>{dialog === 'customer' && <CustomerForm onSubmit={(e) => submit(e, 'customer')} />}{(dialog === 'credit' || dialog === 'payment') && <TransactionForm type={dialog} customers={customers} onSubmit={(e) => submit(e, dialog)} />}{dialog === 'settings' && <SettingsForm apiUrl={apiUrl} accessKey={accessKey} onSave={(url, key) => { setApiUrl(url); setAccessKey(key); localStorage.setItem('credit-ledger-api-url', url); localStorage.setItem('credit-ledger-access-key', key); setDialog(null); notify('보안 연결 정보를 저장했습니다.'); }} />}{dialog === 'detail' && selected && <CustomerDetail customer={selected} items={visibleTransactions} onAction={(type) => setDialog(type)} />}</Modal>}
...
function CustomerDetail({ customer, items, onAction }: { customer: Customer; items: Transaction[]; onAction: (type: 'credit' | 'payment') => void }) { return <div><div className="rounded-2xl bg-secondary/60 p-4"><p className="text-xs text-muted-foreground">현재 받을 금액</p><p className="mt-1 text-2xl font-bold text-ink-red">{money(customer.balance)}</p><p className="mt-2 text-xs text-muted-foreground">{customer.phone}</p></div><div className="mt-3 grid grid-cols-2 gap-2"><Button onClick={() => onAction('credit')} className="h-11"><Plus /> 외상 추가</Button><Button onClick={() => onAction('payment')} variant="outline" className="h-11 text-payment"><ArrowDownLeft /> 입금 처리</Button></div><h3 className="mb-2 mt-6 text-sm font-bold">거래내역</h3><TransactionList items={items} /></div>; }
