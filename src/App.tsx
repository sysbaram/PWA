...
const sampleCustomers: Customer[] = [
  { id: 'C001', name: '김민수', phone: '010-2468-1357', memo: '매월 말일 정산', balance: 485000, oldestDate: '2026-07-03', cylinderCount: 3 },
  { id: 'C002', name: '이정희', phone: '010-9981-2046', memo: '오전 배송 요청', balance: 320000, oldestDate: '2026-08-18', cylinderCount: 1 },
  { id: 'C003', name: '박성호', phone: '010-7102-8821', memo: '', balance: 175000, oldestDate: '2026-09-02', cylinderCount: 0 },
  { id: 'C004', name: '최은영', phone: '010-3366-1290', memo: '현장 담당자 연락', balance: 80000, oldestDate: '2026-09-10', cylinderCount: 2 },
  { id: 'C005', name: '한지우', phone: '010-4877-3301', memo: '', balance: 0, oldestDate: null, cylinderCount: 0 },
];
...
export default function Home() {
  ...
  const [returnAfterCustomer, setReturnAfterCustomer] = useState<'credit' | 'payment' | null>(null);
  ...
  const filtered = customers.filter((c) => `${c.name} ${c.phone} ${c.memo}`.toLowerCase().includes(query.toLowerCase()));
  ...
  function applySummary(data: { customers: Customer[]; transactions: Transaction[]; cylinderRentals?: CylinderRental[] }) {
    const normalizedCustomers = data.customers.map((customer) => ({ ...customer, memo: customer.memo || '', cylinderCount: Number(customer.cylinderCount) || 0 }));
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>, kind: 'customer' | 'credit' | 'payment') {
    ...
    const saved = await persist(kind === 'customer' ? 'addCustomer' : 'addTransaction', { ...payload, type: kind });
    if (saved === false) return;
    if (saved) {
      if (kind === 'customer' && returnAfterCustomer) {
    ...
    if (kind === 'customer') {
      ...
      if (returnAfterCustomer) { setSelected(created); setDialog(returnAfterCustomer); setReturnAfterCustomer(null); notify('고객을 등록하고 자동 선택했습니다.'); return; }
      notify('고객을 등록했습니다.'); setDialog(null); return;
    } else {
  ...
  async function submitCylinder(event: React.FormEvent<HTMLFormElement>, kind: 'rental' | 'return') {
    ...
    const saved = await persist('addCylinderRental', { ...payload, type: kind });
    if (saved === false) return;
    if (saved) { notify(kind === 'rental' ? '가스통 대여를 저장했습니다.' : '가스통 반납을 저장했습니다.'); setDialog(null); return; }
  ...
  async function deleteItem(kind: 'transaction' | 'cylinderRental', id: string) {
    if (!window.confirm('이 기록을 삭제하시겠습니까? 삭제 이력은 AuditLog 시트에 남습니다.')) return;
    const saved = await persist(kind === 'transaction' ? 'deleteTransaction' : 'deleteCylinderRental', { id });
    if (saved === false) return;
    if (saved) { notify('기록을 삭제하고 감사 이력을 저장했습니다.'); return; }
    if (kind === 'transaction') {
      const target = transactions.find((item) => item.id === id);
      setTransactions((list) => list.filter((item) => item.id !== id));
      if (target) setCustomers((list) => list.map((customer) => customer.id === target.customerId ? { ...customer, balance: Math.max(0, customer.balance + (target.type === 'credit' ? -target.amount : target.amount)) } : customer));
    } else {
      const target = cylinderRentals.find((item) => item.id === id);
      setCylinderRentals((list) => list.filter((item) => item.id !== id));
      if (target) setCustomers((list) => list.map((customer) => customer.id === target.customerId ? { ...customer, cylinderCount: Math.max(0, customer.cylinderCount + (target.type === 'rental' ? -target.quantity : target.quantity)) } : customer));
    }
    notify('데모 기록을 삭제했습니다.');
  }
  ...
  return <main className="min-h-screen bg-background pb-28 text-foreground">
    ...
    {dialog && <Modal title={{ customer: '새 고객 등록', credit: '외상 추가', payment: '입금 처리', rental: '가스통 대여', return: '가스통 반납', settings: 'Google Sheets 연결', detail: selected?.name || '거래내역' }[dialog]} onClose={() => { setDialog(null); setReturnAfterCustomer(null); }}>{dialog === 'customer' && <CustomerForm onSubmit={(e) => submit(e, 'customer')} />}{(dialog === 'credit' || dialog === 'payment') && <TransactionForm type={dialog} customers={customers} fixedCustomer={selected} onAddCustomer={() => { setReturnAfterCustomer(dialog); setDialog('customer'); }} onSubmit={(e) => submit(e, dialog)} />}{(dialog === 'rental' || dialog === 'return') && selected && <CylinderForm type={dialog} customer={selected} onSubmit={(e) => submitCylinder(e, dialog)} />}{dialog === 'settings' && <SettingsForm apiUrl={apiUrl} accessKey={accessKey} onSave={(url, key) => { setApiUrl(url); setAccessKey(key); localStorage.setItem('credit-ledger-api-url', url); localStorage.setItem('credit-ledger-access-key', key); setDialog(null); notify('보안 연결 정보를 저장했습니다.'); }} />}{dialog === 'detail' && selected && <CustomerDetail customer={selected} items={visibleTransactions} cylinderItems={visibleCylinderRentals} onAction={(type) => openTransaction(type, selected)} onCylinderAction={(type) => openCylinder(type, selected)} onDeleteTransaction={(id) => deleteItem('transaction', id)} onDeleteCylinder={(id) => deleteItem('cylinderRental', id)} />}</Modal>}
...
function TodayDateInput({ resetKey }: { resetKey: string }) { const [date, setDate] = useState(() => today()); useEffect(() => setDate(today()), [resetKey]); return <Input name="date" type="date" required value={date} onChange={(event) => setDate(event.target.value)} className="h-11" />; }
...
function TransactionForm({ type, customers, fixedCustomer, onAddCustomer, onSubmit }: { type: 'credit' | 'payment'; customers: Customer[]; fixedCustomer: Customer | null; onAddCustomer: () => void; onSubmit: (e: React.FormEvent<HTMLFormElement>) => void }) { return <form onSubmit={onSubmit} className="grid gap-4">{fixedCustomer ? <div><p className="mb-1.5 text-sm font-semibold">고객</p><input type="hidden" name="customerId" value={fixedCustomer.id} /><div className="flex items-center gap-3 rounded-xl border border-primary/20 bg-secondary/65 p-3.5"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-primary font-bold text-primary-foreground">{fixedCustomer.name.slice(0, 1)}</span><span className="min-w-0 flex-1"><strong className="block text-sm">{fixedCustomer.name}</strong><span className="mt-0.5 block text-xs text-muted-foreground">{fixedCustomer.phone || '전화번호 없음'} · 현재 {money(fixedCustomer.balance)}</span></span><span className="rounded-full bg-primary/10 px-2 py-1 text-[10px] font-bold text-primary">선택 고정</span></div></div> : <div className="grid gap-1.5"><div className="flex items-center justify-between"><label htmlFor="transaction-customer" className="text-sm font-semibold">고객</label><button type="button" onClick={onAddCustomer} className="inline-flex items-center gap-1 rounded-lg px-2 py-1 text-xs font-bold text-primary hover:bg-secondary"><Plus className="size-3.5" /> 고객 등록</button></div><select id="transaction-customer" name="customerId" required className="h-11 rounded-lg border border-input bg-background px-3 text-sm">{customers.length === 0 && <option value="">고객을 먼저 등록해 주세요</option>}{customers.map((c) => <option key={c.id} value={c.id}>{c.name} · 현재 {money(c.balance)}</option>)}</select></div>}<Field label={type === 'credit' ? '외상 금액' : '입금 금액'}><Input name="amount" type="number" min="1" required placeholder="0" className="h-11 text-lg font-bold" /></Field><Field label="거래일"><TodayDateInput resetKey={`${type}-${fixedCustomer?.id || 'select'}`} /></Field><Field label="메모"><Input name="memo" placeholder={type === 'credit' ? '품목 또는 외상 사유' : '현금, 계좌이체 등'} className="h-11" /></Field><Button type="submit" size="lg" className={`mt-2 h-12 rounded-xl text-base ${type === 'payment' ? 'bg-payment hover:bg-payment/90' : ''}`}>{type === 'credit' ? '외상 추가하기' : '입금 처리하기'}</Button></form>; }
function CylinderForm({ type, customer, onSubmit }: { type: 'rental' | 'return'; customer: Customer; onSubmit: (e: React.FormEvent<HTMLFormElement>) => void }) { return <form onSubmit={onSubmit} className="grid gap-4"><div><p className="mb-1.5 text-sm font-semibold">고객</p><input type="hidden" name="customerId" value={customer.id} /><div className="flex items-center gap-3 rounded-xl border border-primary/20 bg-secondary/65 p-3.5"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-primary font-bold text-primary-foreground">{customer.name.slice(0, 1)}</span><span className="min-w-0 flex-1"><strong className="block text-sm">{customer.name}</strong><span className="mt-0.5 block text-xs text-muted-foreground">현재 가스통 {customer.cylinderCount}개 대여 중</span></span><span className="rounded-full bg-primary/10 px-2 py-1 text-[10px] font-bold text-primary">선택 고정</span></div></div><Field label="가스통 규격"><Input name="cylinderType" required placeholder="예: LPG 20kg" className="h-11" /></Field><Field label={type === 'rental' ? '대여 수량' : '반납 수량'}><Input name="quantity" type="number" min="1" max={type === 'return' ? customer.cylinderCount : undefined} required placeholder="0" className="h-11 text-lg font-bold" /></Field><Field label={type === 'rental' ? '대여일' : '반납일'}><TodayDateInput resetKey={`${type}-${customer.id}`} /></Field><Field label="메모"><Input name="memo" placeholder="용기 번호 또는 참고사항" className="h-11" /></Field><Button type="submit" size="lg" disabled={type === 'return' && customer.cylinderCount === 0} className={`mt-2 h-12 rounded-xl text-base ${type === 'return' ? 'bg-payment hover:bg-payment/90' : ''}`}>{type === 'rental' ? '대여 기록하기' : customer.cylinderCount === 0 ? '반납할 가스통이 없습니다' : '반납 기록하기'}</Button></form>; }
...
function CustomerDetail({ customer, items, cylinderItems, onAction, onCylinderAction, onDeleteTransaction, onDeleteCylinder }: { customer: Customer; items: Transaction[]; cylinderItems: CylinderRental[]; onAction: (type: 'credit' | 'payment') => void; onCylinderAction: (type: 'rental' | 'return') => void; onDeleteTransaction: (id: string) => void; onDeleteCylinder: (id: string) => void }) { return <div><div className="grid grid-cols-2 gap-2"><div className="rounded-2xl bg-secondary/60 p-4"><p className="text-xs text-muted-foreground">현재 받을 금액</p><p className="mt-1 text-xl font-bold text-ink-red">{money(customer.balance)}</p></div><div className="rounded-2xl bg-warning/10 p-4"><p className="text-xs text-muted-foreground">대여 중인 가스통</p><p className="mt-1 text-xl font-bold text-warning">{customer.cylinderCount}개</p></div></div><div className="mt-2 rounded-xl bg-muted/60 px-3 py-2 text-xs text-muted-foreground"><p>{customer.phone || '전화번호 없음'}</p>{customer.memo && <p className="mt-1 leading-5"><strong className="text-foreground">메모</strong> · {customer.memo}</p>}</div><div className="mt-3 grid grid-cols-2 gap-2"><Button onClick={() => onAction('credit')} className="h-11"><Plus /> 외상 추가</Button><Button onClick={() => onAction('payment')} variant="outline" className="h-11 text-payment"><ArrowDownLeft /> 입금 처리</Button><Button onClick={() => onCylinderAction('rental')} className="h-11 bg-warning hover:bg-warning/90"><Plus /> 가스통 대여</Button><Button onClick={() => onCylinderAction('return')} variant="outline" className="h-11 text-warning">가스통 반납</Button></div><h3 className="mb-2 mt-6 text-sm font-bold">가스통 대여내역</h3><CylinderRentalList items={cylinderItems} onDelete={onDeleteCylinder} /><h3 className="mb-2 mt-6 text-sm font-bold">외상·입금 거래내역</h3><TransactionList items={items} onDelete={onDeleteTransaction} /></div>; }
function TransactionList({ items, onDelete }: { items: Transaction[]; onDelete?: (id: string) => void }) { return <div className="overflow-hidden rounded-[20px] border border-border bg-card">{items.map((item, index) => <div key={item.id} className={`flex items-center gap-3 p-4 ${index ? 'border-t border-border/70' : ''}`}><span className={`grid size-10 place-items-center rounded-xl ${item.type === 'credit' ? 'bg-ink-red/8 text-ink-red' : 'bg-payment/10 text-payment'}`}>{item.type === 'credit' ? <ArrowUpRight className="size-5" /> : <ArrowDownLeft className="size-5" />}</span><span className="min-w-0 flex-1"><strong className="block text-sm">{item.customerName}</strong><span className="mt-0.5 block truncate text-xs text-muted-foreground">{yyyyMmDd(item.date)} · {item.memo || (item.type === 'credit' ? '외상' : '입금')}</span></span><strong className={`text-sm ${item.type === 'credit' ? 'text-ink-red' : 'text-payment'}`}>{item.type === 'credit' ? '+' : '−'}{money(item.amount)}</strong>{onDelete && <button type="button" onClick={() => onDelete(item.id)} aria-label={`${item.customerName} 거래 삭제`} className="grid size-8 shrink-0 place-items-center rounded-lg text-muted-foreground hover:bg-destructive/10 hover:text-destructive"><Trash2 className="size-4" /></button>}</div>)}{items.length === 0 && <div className="p-8 text-center text-sm text-muted-foreground">거래내역이 없습니다.</div>}</div>; }
function CylinderRentalList({ items, onDelete }: { items: CylinderRental[]; onDelete?: (id: string) => void }) { return <div className="overflow-hidden rounded-[20px] border border-border bg-card">{items.map((item, index) => <div key={item.id} className={`flex items-center gap-3 p-4 ${index ? 'border-t border-border/70' : ''}`}><span className={`grid size-10 place-items-center rounded-xl font-bold ${item.type === 'rental' ? 'bg-warning/10 text-warning' : 'bg-payment/10 text-payment'}`}>{item.type === 'rental' ? '+' : '−'}</span><span className="min-w-0 flex-1"><strong className="block text-sm">{item.cylinderType}</strong><span className="mt-0.5 block truncate text-xs text-muted-foreground">{yyyyMmDd(item.date)} · {item.memo || (item.type === 'rental' ? '대여' : '반납')}</span></span><strong className={`text-sm ${item.type === 'rental' ? 'text-warning' : 'text-payment'}`}>{item.type === 'rental' ? '대여' : '반납'} {item.quantity}개</strong>{onDelete && <button type="button" onClick={() => onDelete(item.id)} aria-label={`${item.cylinderType} 기록 삭제`} className="grid size-8 shrink-0 place-items-center rounded-lg text-muted-foreground hover:bg-destructive/10 hover:text-destructive"><Trash2 className="size-4" /></button>}</div>)}{items.length === 0 && <div className="p-8 text-center text-sm text-muted-foreground">가스통 대여내역이 없습니다.</div>}</div>; }
